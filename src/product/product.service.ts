...
import {
  ...
  ProductOptionCategoryDocument,
} from './schemas/productOptionCategory.schema';
import {
  ProductOptionItem,
  ProductOptionItemDocument,
} from './schemas/productOptionItem.schema';
...
export class ProductService {
  constructor(
    ...
    @InjectModel(ProductOptionCategory.name)
    private productOptionCategory: Model<ProductOptionCategoryDocument>,
    @InjectModel(ProductOptionItem.name)
    private productOptionItem: Model<ProductOptionItemDocument>,
  ...
  async createOptionCat(
    dto: CreateProductOptionCategoryDto,
  ...
  async createOptionItem(dto: CreateProductOptionItemDto, image, preview) {
    ...
    const img = this.fileService.createFile(
      FileType.Image,
      image,
      productArticul,
      title,
    ...
    const dir = img[1];
    const prev = this.fileService.createFile(
      FileType.Preview,
      preview,
      productArticul,
      title,
    ...
    const createdOption = await this.productOptionItem.create({
      ...dto,
      preview: prev[0],
      image: img[0],
    });

    const optionCategory = await this.productOptionCategory.findById(
      dto.optionCategoryID,
    );
    if (!optionCategory) {
